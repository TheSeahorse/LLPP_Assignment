...
__global__
void heatIntensify(int *d_heatmap, int size)
{
    int row = blockIdx.y;// * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    {
        d_heatmap[row*size+col] = (int)round(d_heatmap[row*size+col] + 0.40);
    }
}
...
void setMaxHeat(int *d_heatmap, int size)
{
    ...
    {
        d_heatmap[row*size+col] = d_heatmap[row*size+col];
    }
    else
    {
...
void updateHeatIntensity(int *heatmap, int SIZE)
{
	...
	cudaMemcpy(d_heatmap, heatmap, SIZE*SIZE*sizeof(int), cudaMemcpyHostToDevice);
	heatIntensify<<<4,256>>>(d_heatmap, SIZE*SIZE);
	cudaMemcpy(heatmap, d_heatmap, SIZE*SIZE*sizeof(int), cudaMemcpyDeviceToHost);
    cudaFree(d_heatmap);
}
...
void updateSetMaxHeat(int *heatmap, int SIZE)
{
    ...
    cudaMalloc(&d_heatmap, SIZE*SIZE*sizeof(int));
	cudaMemcpy(d_heatmap, heatmap, SIZE*SIZE*sizeof(int), cudaMemcpyHostToDevice);
	setMaxHeat<<<4,256>>>(d_heatmap, SIZE);
	cudaMemcpy(heatmap, d_heatmap, SIZE*SIZE*sizeof(int), cudaMemcpyDeviceToHost);
    cudaFree(d_heatmap);
}
