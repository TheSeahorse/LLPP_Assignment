...
void Ped::Model::setupHeatmapCuda()
{
  ...
  allocate(heatmap, blurred_heatmap, SIZE, SCALED_SIZE);
}
...
{   
  ...
  for (int i = 0; i < agents.size(); i++) {
    destinationsX[i] = agents[i]->getDesiredX();
    destinationsY[i] = agents[i]->getDesiredY();
  }

  update_heatmap(heatmap, scaled_heatmap, blurred_heatmap, hm, shm, bhm, destinationsX, destinationsY, SIZE, SCALED_SIZE, agents.size());
